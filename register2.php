...
// Check for a password and match against the confirmed passwodrd:
    if (preg_match('/^\w{4,20}$/', $trimmed['password1'])) {
        if ($trimmed['password1'] == $trimmed['password2']) { //If the two passwords match
            $p = mysqli_real_escape_string($db, $trimmed['password2']);
        } else {
    ...
    if ($fn && $ln && $e &&p) {
        ...
        if(mysqli_num_rows($r) == 0 ) {
            ...
            if (mysqli_affected_rows($dbc) == 1) {
                ...
                // Send the email: 
                $body = "Thank your requesting at our site. To activate your account , please click on this link: \n\n";
...
?>
        ...
        <p><b>Password:</b> <input type= "password" name="password1" size="20" maxlength="20" /> <small>Use only
